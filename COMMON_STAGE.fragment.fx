...
//#define BLACK_COL vec3(16,21,25)/255.0
#define BLACK_COL vec3(75,75,75)/255.0
#define WHITE_COL vec3(200,200,200)/255.0
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 uv = (fragCoord.xy-vec2(1024.0,1024.0)*0.5)/1024.0;
    vec2 uvT = vUV;
    vec4 tex = texture2D(textureSampler, uvT);

    float an = 0.2;
    float s = sin(an);
    float c = cos(an);
    uv = (uv-0.5) * mat2(c, s, -s, c);
    uv += 0.5;
    ...
    float m = m1*m2 * ((s1*s2*s3) * (1.-l)) * sh;
    //float m = ((s1*s2*s3) * (1.-l)) * sh;
    //m = clamp(m, 0., 1.);
    
    vec3 col = WHITE_COL;
    
    if(uv.y > -0.9){
        col = mix(WHITE_COL, (0.5 + 0.5*cos(iTime+uv.xyx*3.+vec3(0,2,4))), m);    
    }
...
void main() 
{
    ...
    mainImage(gl_FragColor, vUV * vec2(1024.0,1024.0));
}